...
app.post('/api/ai/generate-trip-rag', auth, async (req, res) => {
  ...
  console.log('🔑 使用API Key:', process.env.BAILIAN_API_KEY ? process.env.BAILIAN_API_KEY.substring(0, 10) + '...' : '未设置');
  
  try {
    console.log('🤖 开始尝试直接AI调用...');
...
8. 考虑当地交通、天气、开放时间等实际因素`;
    ...
    // 解析JSON响应
    const parsedResponse = parseAIResponse(aiResponse, destination, travelers, startDate, endDate, budget, preferences);
...
// 降级模式生成每日活动函数
function generateDayActivitiesFallback(destination, dayIndex, totalDays, dailyBudget, preferences) {
  const activities = [];
  ...
  if (dayIndex === 0) {
    ...
    activities.push({
      time: '下午',
      title: '当地美食体验',
      description: `品尝${destination}特色美食，体验当地文化`,
      location: '市中心餐厅',
    ...
    activities.push({
      time: '上午',
      title: '城市探索',
      description: `游览${destination}著名景点，拍照留念`,
      location: '市中心景点',
...
function getDayTitle(dayIndex, totalDays, destination) {
  if (dayIndex === 0) {
    ...
    return `离开${destination}`;
  } else {
    return `探索${destination}`;
  }
}
...
async function generateFallbackTrip(userInput) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const dayActivities = generateDayActivitiesFallback(destination, i, days, dailyBudget, preferences);
...
- 确保JSON结构完整，无语法错误`;
    ...
    // 解析AI响应
    let parsedResponse = await parseAIResponse(aiResponse, destination, travelers, startDate, endDate, budget, preferences);
    
    async function parseAIResponse(aiResponse, destination, travelers, startDate, endDate, budget, preferences) {
      ...
      console.log('清理后的响应前500字符:', cleanResponse.substring(0, 500));
    ...
    // 生成每日活动 - 使用已定义的函数
    function generateDayActivitiesInternal(destination, dayIndex, totalDays, dailyBudget, preferences) {
      const activities = [];
